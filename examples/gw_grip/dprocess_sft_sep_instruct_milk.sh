...
# output files
#SBATCH --output=dprocess-sft-sep-inst-milk_%A_%a.out
#SBATCH --error=dprocess-sft-sep-inst-milk_%A_%a.err
...
python -u $SCRIPT_DIR/../../semantic_state_estimator/eval/process_datapoints.py \
--data_dir="data_dir" \
--domain="$SCRIPT_DIR/domain.pddl" \
--problem="$SCRIPT_DIR/problem.pddl" \
--separate_images \
--out_dir="72B SFT SEP Instruct-milk" \
--vqa_model_id="lmms-lab/llava-onevision-qwen2-72b-ov-sft" \
