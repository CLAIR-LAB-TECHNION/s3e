...
class EpisodeRunner:
    def __init__(
        ...
        self.episodes_dir = os.path.join(data_dir, EPISODES_DIR)
    ...
    def sample_goal(self, state: UPState, task_horizon: int) -> UPState:
        ...
        """
        applicable_actions = self.problem_sim.get_applicable_actions(state)
        goal_state = state
        for _ in range(task_horizon):
            ...
            goal_state = self.problem_sim.apply_action(goal_state, action, params)
            applicable_actions = self.problem_sim.get_applicable_actions(goal_state)

        return goal_state

    def get_next_action(self, pred_state, goal_state):
        # set problem initial state
        set_problem_init_state(self.problem, pred_state)
        ...
        # set problem goal state
        set_problem_goal_state(self.problem, goal_state)
    ...
    def apply_action(self, action):
        ...
        action_name, params = action.action.name, action.params
    ...
    def save_render(self, step_id, render):
    ...
    def save_trajectory_data(
        ...
        prev_obs_fname = self.save_render(step_id - 1, prev_obs)
        next_obs_fname = self.save_render(step_id, next_obs)

        # save trajectory data
        step_fname = os.path.join(
            self.episodes_dir, TRAJECTORY_STEP_FNAME_FORMAT.format(step_id) + ".json"
        )
    ...
    def shift_trajcectory_and_save(
        ...
        reached_goal = state_dict == goal_state_dict
        predicted_goal = pred_state_dict == goal_state_dict
    ...
    def run_episode(self, task_horizon: int, out_dir: str):
        ...
        while (
            ...
            # get next action
            action = self.get_next_action(pred_state, goal_state)
            if action is None:
                print("no plan from current predicted state. resetting")
                if action_count == 0:  # no actions taken. save initial state
                    self.save_trajectory_data(
                        action_count,
                        action,
                        cur_obs_renders,
